...
public class JwtProvider {
    ...
    @Value("${jwt.secret:TextoMuyLargoQueEditoParaSerOriginalYQueNoTengaLoMismoQueVienePorDefectoDelGitDeMontseLaProfesora}")
    ...
    public String generateToken(Authentication authentication) {
        ...
        return Jwts.builder().signWith(Keys.hmacShaKeyFor(jwtSecreto.getBytes()), SignatureAlgorithm.HS512).setHeaderParam("typ", TOKEN_TYPE).setSubject(Long.toString(user.getId())).setIssuedAt(new Date()).setExpiration(tokenExpirationDate).claim("fullname", user.getUsername()).claim("roles", user.getRol()).compact();
    }
